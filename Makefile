...
bumpversion: CURRENT_VERSION?=$(shell echo `sed -n -e 's/^version *= *\(.*\)/\1/p' default/app.conf`)
...
build:
	...
	tar czvf -C $(TARGET_WORKING_DIR) $(TARGET_ARCHIVE_DIR)/$(NAME).spl $(NAME)

buildbeta:
	git archive --format $(BETA_FORMAT) --prefix=$(NAME)/ --output $(TARGET_ARCHIVE_DIR)/$(NAME).$(BETA_FORMAT) $(BETA_BRANCH)
