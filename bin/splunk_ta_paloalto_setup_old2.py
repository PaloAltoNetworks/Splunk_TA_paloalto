...
class ConfigApp(admin.MConfigHandler):
    ...
    def handleEdit(self, confInfo):
        ...
        logger.info("Here are the settings")
        logger.info(all_origin_settings)
        logger.info(all_settings)
        logger.info(customized_conf_file.get_all())
        ...
        customized_conf_file = get_or_create_conf_file(conf_mgr, setup_const.myta_customized_conf)
        self._updateConfStanzas(all_settings.get(setup_const.myta_customized_settings, {}), customized_conf_file, self.encrypt_fields_customized, all_settings)
        ...
        for cred, conf_file in self.cred_confs:
            ...
            if creds == setup_const.ignore_backend_req:
                ...
                continue
            if creds:
                self._updateConfStanzas(creds, cred_conf_file, self.encrypt_fields_credential, all_settings)
    ...
    def _updateConfStanzas(self, all_settings, conf_file, encrypt_fields, all_origin_settings):
        logger.info("Getting Original Settings:")
        logger.info(all_origin_settings)
        logger.info("all_settings:")
        logger.info(all_settings)
        ...
        logger.info("Updated_stanza:")
        logger.info(updated_stanzas)

        # for s in all_settings.keys():
        #     if s not in updated_stanzas:
        #         console.log(s)

        to_be_deleted_stanzas = [ s for s in all_origin_settings if s not in updated_stanzas ]
        logger.info("To be Deleted: ")
        logger.info(to_be_deleted_stanzas)
