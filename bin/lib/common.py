...
class SplunkConnector(object):
    ...
    def get_firewall_credentials(self):
        """Given a splunk self.session_key returns a clear
        text user name and password from a splunk password container"""
        try:
            # Get all credentials
            ...
            accounts = entity.getEntities(['admin',
                                           'Splunk_TA_paloalto_account'],
                                          namespace=self.APPNAME,
                                          owner='nobody',
                                          sessionKey=self.session_key)
        except Exception as e:
            self.exit_with_error('Could not get %s credentials from '
        ...
        if username and password:
            self.logger.debug('Credentials have been found')
    ...
    def get_wildfire_apikey(self):
        ...
        try:
            self.logger.debug("Getting wildfire apikey from Splunk")
            entities = entity.getEntities(['admin', 'passwords'],
                                          namespace=self.APPNAME,
                                          owner='nobody',
                                          sessionKey=self.session_key)
        ...
        if ('__REST_CREDENTIAL__#Splunk_TA_paloalto#configs/conf-splunk_ta_paloalto_settings:additional_parameters``splunk_cred_sep``1:' in entities):
            ...
            if 'wildfire_api_key' in clear_password:
                ...
                self.logger.debug('Password exist')
                return api_key
            else:
                self.exit_with_error("No Wildfire API key is set, "
    ...
    def get_firewall_apikey(self):
        """Given a splunk self.session_key returns a clear text
        API Key from a splunk password container"""
        try:
            entities = entity.getEntities(['admin', 'passwords'],
                                          namespace=self.APPNAME,
                                          owner='nobody',
    ...
    def set_firewall_apikey(self, apikey):
        """Given a splunk self.session_key sets the firewall API
        key in the Splunk password store"""
        try:
            '''The password cannot be modified, so it must be
            deleted before it can be added back.'''
            ...
            entity.setEntity(apikey_entity, sessionKey=self.session_key,
                             strictCreate=False)
        except Exception as e:
            stack = traceback.format_exc()
            self.logger.warn(stack)
            self.logger.warn("entity exception")
            self.exit_with_error("Could not set %s firewall apikey from "
                                 "splunk. Error: %s" % (self.APPNAME, str(e)))

    def delete_firewall_apikey(self):
        """Given a splunk self.session_key delete the firewall
        API key in the Splunk password store"""
        try:
            ...
            self.exit_with_error("Could not delete %s firewall apikey from "
                                 "splunk. Error: %s" % (self.APPNAME, str(e)))
    ...
    def apikey(self, hostname):
        ...
        try:
            ...
            try:
                ...
                '''If Splunk doesn't know the API Key, get the
                username and password instead'''
                self.logger.debug("Getting credentials from"
                                  "Splunk credential store")
                ...
                device = pandevice.base.PanDevice(hostname,
                                                  fw_username, fw_password)
                apikey = device.api_key
                '''Save the API key to the Splunk
                credential store inside the App'''
                self.logger.debug("Adding API Key to Splunk credential store")
                self.set_firewall_apikey(apikey)
                return apikey
            except NoCredentialsFound as e:
                self.exit_with_error("No Firewall/Panorama credentials for "
                                     "searchbar command. Please set the "
                                     "username and password in the Add-on "
                                     "set up page.", log_traceback=False)
            except Exception as e:
                self.exit_with_error("Unable to get apikey from "
                                     "firewall: %s" % str(e))
    ...
    def add_firewall_cli_args(self, parser):
        ...
        fw_group.add_argument('-c', '--splunk-creds',
                              action='store_true',
